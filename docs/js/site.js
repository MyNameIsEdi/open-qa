...
/** Copies the raw system prompt text shown in the slide-over panel. */
function copyPanelPrompt() {
  const text = document.getElementById('panelCode').textContent.trim();
  const btn  = document.getElementById('panelCopyPromptBtn');
  if (!text || !btn) return;
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).then(onSuccess).catch((e) => clipboardFallback(text, onSuccess));
  } else {
...
/**
 * Downloads the currently viewed skill as a Claude-optimised SKILL.md file.
 * Wraps the prompt in Anthropic-recommended <system> / <instructions> XML tags.
 */
function downloadSkillMD() {
  if (!_panelCard) return;
  const title   = _panelCard.dataset.title       || 'skill';
  const desc    = _panelCard.dataset.description || '';
  const command = _panelCard.dataset.command     || '';
  const prompt  = _panelCard.dataset.prompt      || '';
  if (!prompt) return;
  ...
  const lines = [
    `<system>`,
    `You are an expert QA Architect specializing in AI-assisted software testing.`,
    `</system>`,
    ``,
    `<instructions>`,
    prompt,
    ...
    `**Skill:** ${title}  `,
    `**Description:** ${desc}  `,
    command ? `**CLI:** \`${command}\`  ` : null,
    ``,
    `_Generated by [Intelligent Testing Toolkit](https://github.com/MyNameIsEdi/intelligent-testing-toolkit)_`,
  ...
  const btn = document.getElementById('panelDownloadBtn');
  if (btn) {
    const icon = btn.querySelector('.material-symbols-outlined');
    if (icon) {
      icon.textContent = 'check';
