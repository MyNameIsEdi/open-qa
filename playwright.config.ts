...
const rc: RC = process.env.PW_RUNTIME_CONFIG
  ? JSON.parse(process.env.PW_RUNTIME_CONFIG)
  : {}
...
function buildProjects(b?: RC['browsers']) {
  ...
  if (!b || b.chromium !== false) projects.push({ name: 'chromium', use: { ...devices['Desktop Chrome'] } })
