...
const PIPELINE = [
  { icon: '💻', label: 'Code Push',    color: '#6366f1', delay: 0.10 },
  { icon: '🔨', label: 'Build',        color: '#3b82f6', delay: 0.40 },
  { icon: '🧪', label: 'Test',         color: '#22c55e', delay: 0.70 },
  { icon: '🚀', label: 'Deploy',       color: '#f97316', delay: 1.00 },
]
...
const TEST_HOOKS = [
  ...
  { stage: 'On deploy',        type: 'Sanity / E2E',     color: '#f97316', delay: 1.90 },
]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {PIPELINE.map((s, i) => (
          <div key={s.label} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            <div style={{
              ...
              <p style={{ fontSize: 9.5, fontWeight: 700, color: s.color, margin: '3px 0 0' }}>{s.label}</p>
            </div>
            {i < PIPELINE.length - 1 && (
              <div style={{
                animation: `qa-arrowIn .3s ease ${s.delay + 0.25}s both`,
                fontSize: 14, color: '#cbd5e1', padding: '0 3px', transformOrigin: 'left',
              }}>→</div>
            )}
          </div>
        ))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', margin: 0,
          textTransform: 'uppercase', letterSpacing: '0.07em' }}>Where tests plug in</p>
        {TEST_HOOKS.map(h => (
          <div key={h.stage} style={{
            animation: `qa-stageIn .35s ease ${h.delay}s both`,
            display: 'flex', alignItems: 'center', gap: 10, padding: '7px 11px', borderRadius: 9,
            background: h.color + '10', border: `1.5px solid ${h.color}33`,
          }}>
            <span style={{
              animation: `qa-checkPop .35s ease ${h.delay + 0.2}s both`,
              fontSize: 14,
            }}>✅</span>
            <span style={{ fontSize: 10.5, color: '#374151', flex: 1 }}><strong>{h.stage}</strong> → {h.type}</span>
            <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 99,
              background: h.color, color: '#fff' }}>{h.type.split(' ')[0]}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        background: '#0f172a', borderRadius: 10, padding: '10px 13px',
        ...
        <div style={{ display: 'flex', gap: 5, marginBottom: 7 }}>
          ...
          <span style={{ fontSize: 9, color: '#475569', marginLeft: 4 }}>.github/workflows/test.yml</span>
        </div>
