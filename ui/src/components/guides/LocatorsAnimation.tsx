...
const LOCATORS = [
  ...
  },
  {
    ...
    color: '#84cc16',
    star: '★★★★☆',
    tip: 'Great for form inputs',
  },
  {
    rank: 3,
    name: 'getByTestId()',
    example: "getByTestId('login-btn')",
    bar: '60%',
    color: '#eab308',
    star: '★★★☆☆',
    tip: 'Add data-testid in HTML',
  },
  {
    rank: 4,
    name: 'getByText()',
    ...
    color: '#f97316',
    star: '★★☆☆☆',
    tip: 'OK — breaks on text change',
  },
  {
    rank: 5,
    name: 'locator(css)',
    example: "locator('.btn-primary > span')",
    bar: '20%',
    color: '#ef4444',
    star: '★☆☆☆☆',
    tip: 'Avoid — breaks on redesign',
  },
];
...
export function LocatorsAnimation() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
      ...
      {LOCATORS.map((l, i) => (
        <div key={l.rank} style={{ animation: `qa-loc .4s ease ${i * 0.3 + 0.1}s both` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <span style={{ fontSize: 10, fontWeight: 800, color: l.color, width: 14 }}>
              {l.rank}
            </span>
            <span style={{ fontSize: 11.5, fontWeight: 700, color: '#1f2937', flex: 1 }}>
              {l.name}
            </span>
          ...
          >
            <div
              style={{
                ...
                background: l.color,
                width: l.bar,
                animation: `qa-bar .5s ease ${i * 0.3 + 0.4}s both`,
                ['--w' as string]: l.bar,
              }}
            />
          </div>
