...
const ROWS = [
  { label: 'Regression tests',      ok: true,  note: 'Run every deploy — ROI in days',        delay: 0.10 },
  { label: 'Smoke tests',           ok: true,  note: 'Fast safety net after each build',       delay: 0.45 },
  { label: 'Multi-browser checks',  ok: true,  note: 'Playwright runs Chromium, Firefox, WebKit', delay: 0.80 },
  { label: 'Exploratory testing',   ok: false, note: 'Needs human curiosity & judgment',       delay: 1.15 },
  { label: 'One-time test',         ok: false, note: 'Manual is faster — skip automation',     delay: 1.50 },
]
...
export function WhenToAutomateAnimation() {
  return (
    <div className="w-full">
      <style>{css}</style>
      <p style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 8,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {ROWS.map(r => (
          ...
          >
            <span style={{ fontSize: 20, display: 'block', animation: `qa-ico .35s ease ${r.delay + 0.28}s both` }}>
              {r.ok ? '✅' : '❌'}
            </span>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: '#1f2937', margin: 0 }}>{r.label}</p>
