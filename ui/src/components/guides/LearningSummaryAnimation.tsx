...
const MODULES_DONE = [
  { icon: '🎓', label: 'Welcome & QA Foundations',        delay: 0.10 },
  ...
  { icon: '🗄️', label: 'SQL for QA — SELECT to JOINs',     delay: 1.00 },
  { icon: '🧠', label: 'AI, DevOps & Cybersecurity',        delay: 1.18 },
  { icon: '🏁', label: 'Interview Prep & Next Steps',       delay: 1.36 },
]

const NEXT_30 = [
  { day: 'Week 1', task: 'Write a Playwright test suite for a live website',  color: '#3b82f6', delay: 1.6  },
  { day: 'Week 2', task: 'Build & run an API test collection in Postman',     color: '#22c55e', delay: 1.85 },
  { day: 'Week 3', task: 'Write 10 QA SQL queries against a real database',   color: '#f97316', delay: 2.10 },
  { day: 'Week 4', task: 'Push everything to GitHub — your first QA portfolio', color: '#a855f7', delay: 2.35 },
]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 36, animation: 'qa-confetti 1.2s ease-in-out 1.5s infinite alternate' }}>🎓</div>
      ...
      {/* Module checklist */}
      <div>
        <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', margin: '0 0 7px',
          ...
          {MODULES_DONE.map((m, i) => (
            <div key={i} style={{
              ...
              <span style={{
                ...
                fontSize: 13, color: '#16a34a', flexShrink: 0,
              }}>✅</span>
              <span style={{ fontSize: 10, color: '#1f2937' }}>{m.icon} {m.label}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {NEXT_30.map((n, i) => (
            <div key={i} style={{
              ...
              background: n.color + '0f', border: `1.5px solid ${n.color}33`,
            }}>
              <span style={{
                fontSize: 9, fontWeight: 800, padding: '3px 8px', borderRadius: 99,
                background: n.color, color: '#fff', whiteSpace: 'nowrap',
              }}>{n.day}</span>
      ...
      <div style={{
        ...
        background: '#fef9c3', border: '1px solid #fde047', textAlign: 'center',
      }}>
