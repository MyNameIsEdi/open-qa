  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
        <div style={{ flex: 1, textAlign: 'center', fontSize: 10.5, fontWeight: 700, color: '#ef4444',
          animation: 'qa-fadeIn .4s ease .1s both' }}>
          ❌ Without POM
        </div>
        <div style={{ width: 24 }} />
        <div style={{ flex: 1, textAlign: 'center', fontSize: 10.5, fontWeight: 700, color: '#22c55e',
          animation: 'qa-fadeIn .4s ease .1s both' }}>
          ✅ With POM
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        {/* Before */}
        <div style={{ flex: 1, background: '#fff1f2', border: '1.5px solid #fecdd3', borderRadius: 10, padding: '10px 12px' }}>
          {BEFORE_TESTS.map((t, i) => (
            <div key={i} style={{
              animation: `qa-fadeIn .35s ease ${i * 0.2 + 0.2}s both`,
              fontSize: 9.5, fontFamily: 'monospace', color: '#9f1239',
              marginBottom: 5, lineHeight: 1.5,
            }}>{t}</div>
          ))}
          <div style={{ animation: 'qa-fadeIn .35s ease .9s both', fontSize: 9, color: '#be123c', marginTop: 4, fontStyle: 'italic' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            background: '#1e293b', borderRadius: 9, padding: '8px 11px', marginBottom: 7,
            fontFamily: 'monospace', fontSize: 9.5,
          }}>
            {POM_CLASS.map((l, i) => (
              <div key={i} style={{ color: i === 0 ? '#93c5fd' : '#94a3b8' }}>{l}</div>
            ))}
          </div>
          {/* Tests using POM */}
          <div style={{ background: '#f0fdf4', border: '1.5px solid #bbf7d0', borderRadius: 10, padding: '10px 12px' }}>
            {AFTER_TESTS.map((t, i) => (
              <div key={i} style={{
                ...
                fontSize: 9.5, fontFamily: 'monospace', color: '#166534',
                marginBottom: 5, lineHeight: 1.5,
              }}>{t}</div>
            ))}
            <div style={{ animation: 'qa-slideR .35s ease 2.2s both', fontSize: 9, color: '#15803d', marginTop: 4, fontStyle: 'italic' }}>
              ↑ one place to update
            </div>
          </div>
        </div>
      ...
      <div style={{
        ...
        background: '#eff6ff', border: '1px solid #bfdbfe', fontSize: 10.5,
