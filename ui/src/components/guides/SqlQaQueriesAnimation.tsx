...
export function SqlQaQueriesAnimation() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <style>{css}</style>
      <p style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--text-muted)', margin: '0 0 2px',
        ...
        {QUERIES.map(q => (
          <div key={q.title} style={{
            ...
            <div style={{
              ...
              padding: '6px 10px', background: q.tagColor + '12',
              ...
              <span style={{
                ...
                background: q.tagColor, color: '#fff',
              }}>{q.tag}</span>
            </div>
            {/* SQL */}
            <div style={{
              ...
              {q.sql.split('\n').map((line, i) => (
                <div key={i} style={{ color: /^(SELECT|FROM|WHERE|GROUP|HAVING|LEFT|ON)/.test(line.trim()) ? '#93c5fd' : '#94a3b8' }}>{line}</div>
              ))}
            </div>
          </div>
        ))}
