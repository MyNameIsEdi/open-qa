...
import {
  KeyRound, Bot, Save, Plus, Trash2, ChevronDown, ChevronUp,
  Eye, EyeOff, Check, AlertTriangle, LayoutDashboard,
  Wifi, WifiOff, RefreshCw, Server,
} from 'lucide-react'
import {
  useSettings,
  DEFAULT_AGENTS,
...
const POPULAR_OLLAMA_MODELS = [
  ...
  'codellama', 'deepseek-coder', 'deepseek-r1', 'phi3',
  'qwen2.5-coder', 'gemma2', 'nomic-embed-text',
]
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-start gap-3 py-3 border-b last:border-0"
      ...
      <div className="sm:w-48 shrink-0">
        ...
        {desc && <p className="text-[11px] mt-0.5 leading-snug" style={{ color: 'var(--text-muted)' }}>{desc}</p>}
      </div>
...
function OllamaStatusBadge({ status, version }: { status: OllamaStatus; version?: string }) {
  ...
  if (status === 'ok') return (
    <span className="flex items-center gap-1 text-[11px] text-emerald-500">
      <Wifi size={11} />{version ? `Connected · v${version}` : 'Connected'}
    </span>
  )
  return (
    <span className="flex items-center gap-1 text-[11px] text-red-400">
      <WifiOff size={11} />Not reachable
    </span>
  )
}
...
function AgentEditor({
  agent, deskIds, onChange, onRemove,
}: {
  agent:    AgentConfig
  deskIds:  string[]
  ...
  const inputCls   = 'w-full px-3 py-1.5 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-400'
  const inputStyle = {
  ...
  return (
    <div className="rounded-xl overflow-hidden"
      ...
      <button className="w-full flex items-center gap-3 px-4 py-3 text-left transition-opacity hover:opacity-80"
        onClick={() => setOpen(o => !o)}>
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 space-y-3 border-t" style={{ borderColor: 'var(--border)' }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold mb-1" style={{ color: 'var(--text-muted)' }}>Name</label>
              <input type="text" value={agent.name}
                onChange={e => onChange({ name: e.target.value })}
                className={inputCls} style={inputStyle} placeholder="Agent name" />
            </div>
            <div>
              <label className="block text-[11px] font-semibold mb-1" style={{ color: 'var(--text-muted)' }}>Role</label>
              <input type="text" value={agent.role}
                onChange={e => onChange({ role: e.target.value })}
                className={inputCls} style={inputStyle} placeholder="E.g. E2E Automation" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold mb-1" style={{ color: 'var(--text-muted)' }}>Character Sprite</label>
              <select value={agent.characterSprite}
            ...
            <div>
              <label className="block text-[11px] font-semibold mb-1" style={{ color: 'var(--text-muted)' }}>Assigned Desk</label>
              <select value={agent.deskId}
                onChange={e => onChange({ deskId: e.target.value })}
                className={inputCls} style={inputStyle}>
                <option value="">— unassigned —</option>
                {deskIds.map(id => <option key={id} value={id}>{id}</option>)}
              </select>
            </div>
          </div>
...
export default function SettingsPage() {
  const {
    ...
    agents, updateAgent, addAgent, removeAgent, setAgents,
    layout, updateLayout,
  } = useSettings()
  ...
  const handleAddAgent = () => {
    addAgent({
      ...
      name:            'New Agent',
      role:            'QA Engineer',
      systemPrompt:    'You are a senior QA engineer. Analyse the input and provide clear, actionable insights.',
  ...
  const inputCls   = 'w-full px-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400'
  const inputStyle = {
    border:          '1px solid var(--border)',
    backgroundColor: 'var(--bg-body)',
    color:           'var(--text-main)',
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
          Configure your AI provider, API keys, and QA agent personalities.
        ...
        <SectionCard icon={<KeyRound size={14} />} title="AI Provider">
          ...
          <div className="flex gap-2 p-1 rounded-xl mb-5"
            ...
            <button
              ...
              style={settings.provider === 'gemini'
                ? { background: '#1e40af', color: '#fff', boxShadow: '0 2px 8px rgba(30,64,175,0.4)' }
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-all"
              style={settings.provider === 'ollama'
                ? { background: '#065f46', color: '#fff', boxShadow: '0 2px 8px rgba(6,95,70,0.4)' }
                : { color: 'var(--text-muted)' }
              }>
              🦙 Local Ollama
            </button>
              ...
              <Field
                ...
                {settings.geminiApiKey
                  ? <p className="flex items-center gap-1 mt-1.5 text-[11px] text-emerald-500">
                      <Check size={11} />Key present — {settings.geminiApiKey.length} characters
                    </p>
                  : <p className="flex items-center gap-1.5 mt-1.5 text-[11px]" style={{ color: 'var(--text-muted)' }}>
                      <AlertTriangle size={11} />
                      Get a free key at{' '}
                      <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer"
                        className="underline text-blue-400">aistudio.google.com</a>
                    </p>
                }
              </Field>
          ...
          {settings.provider === 'ollama' && (
            <>
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl mb-4 text-[12px]"
                style={{ background: 'rgba(6,95,70,0.1)', border: '1px solid rgba(16,185,129,0.2)', color: 'var(--text-main)' }}>
                <Server size={14} className="shrink-0 mt-0.5 text-emerald-400" />
                <div>
                  <p className="font-semibold text-emerald-400">Local Ollama — 100% private, no API key needed</p>
                  <p className="mt-0.5 leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                    Install from <a href="https://ollama.com" target="_blank" rel="noopener noreferrer"
                      className="underline text-blue-400">ollama.com</a>, then run{' '}
                    <code className="px-1 py-0.5 rounded font-mono text-[11px]"
                      style={{ background: 'var(--bg-muted)' }}>ollama pull llama3.2</code> and{' '}
                    <code className="px-1 py-0.5 rounded font-mono text-[11px]"
                      style={{ background: 'var(--bg-muted)' }}>ollama serve</code>.
                    Image attachments are not supported with Ollama (text only).
                  </p>
                </div>
              </div>

              <Field label="Ollama Base URL" desc="The address where your Ollama server is listening.">
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={settings.ollamaBaseUrl}
                    onChange={e => { updateSettings({ ollamaBaseUrl: e.target.value }); setOllamaStatus('idle') }}
                  ...
                  <button
                    ...
                    {ollamaStatus === 'testing' ? <RefreshCw size={12} className="animate-spin" /> : 'Test'}
                  </button>
                </div>
              ...
              <Field label="Ollama Model" desc="The model to use for chat. Pull models with: ollama pull <name>">
                <div className="flex gap-2">
                  <input
                    type="text"
                  ...
                  <button
                    ...
                    {availableModels.length > 0 ? `${availableModels.length} found` : 'Fetch models'}
                  </button>
                </div>
                <datalist id="ollama-models-list">
                  {(availableModels.length > 0 ? availableModels : POPULAR_OLLAMA_MODELS).map(m => (
        ...
        <SectionCard icon={<Bot size={14} />} title="Agent Configuration">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              ...
              expand to edit system prompts, roles, and desk assignments.
            </p>
            <button
        ...
        <SectionCard icon={<LayoutDashboard size={14} />} title="Office Layout">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {layout.desks.map(desk => {
              ...
              return (
                <div key={desk.id} className="flex flex-col gap-1 p-3 rounded-xl text-[11px]"
                  style={{
                    backgroundColor: occupant ? 'rgba(59,130,246,0.08)' : 'var(--bg-body)',
                    border:          `1px solid ${occupant ? '#1e40af' : 'var(--border)'}`,
                  }}>
                  <span className="font-bold font-mono" style={{ color: 'var(--text-main)' }}>{desk.id}</span>
                  <span style={{ color: 'var(--text-muted)' }}>{desk.label}</span>
                  <span style={{ color: 'var(--text-muted)' }}>col {desk.x}, row {desk.y}</span>
                  {occupant
