...
async function loadFurniture(base: string, catalog: { id: string; furniturePath: string; width: number; height: number }[]) {
...
export async function loadOfficeAssets(): Promise<void> {
  ...
  const [assetIndex, catalog] = await Promise.all([
    ...
    fetch(`${safeBase}assets/furniture-catalog.json`).then((r) => r.json()) as Promise<
      { id: string; furniturePath: string; width: number; height: number; label: string; category: string; footprintW: number; footprintH: number; isDesk: boolean; canPlaceOnWalls: boolean; groupId?: string; orientation?: string; state?: string; canPlaceOnSurfaces?: boolean; backgroundTiles?: number; mirrorSide?: boolean; rotationScheme?: string; animationGroup?: string; frame?: number }[]
    >,
  ])
...
export async function loadDefaultLayout(): Promise<unknown | null> {
  ...
  const safeBase = base.endsWith('/') ? base : `${base}/`
  try {
    const index = await fetch(`${safeBase}assets/asset-index.json`).then((r) => r.json()) as { defaultLayout: string | null }
